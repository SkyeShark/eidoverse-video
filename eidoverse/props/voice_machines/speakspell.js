// speakspell.js — the 1978 educational speaking toy (red-orange handheld speller, 8-character VFD), for DAISY's
// museum of voices. Era marks as homage, drawn by us (blender/era_logos.py, baked into the card): the bouncy
// Speak & Spell wordmark, TEXAS INSTRUMENTS in small caps and the Texas-outline "ti" mark.
//
//   const { build } = await import(new URL('props/voice_machines/speakspell.js', EIDOVERSE_DIR).href);
//   const ss = await build(THREE, { text: 'LOVE' });
//   scene.add(ss.group);
//   ss.parts.display.setText('LOVE');                    // ≤ 8 chars, 14-segment VFD, unlit segments ghosted ~8 %
//   // per frame: ss.update(t, { power: 0..1, voice: 0..1 })
//   //   power = VFD lit;  voice = this machine's own voice loudness → the VFD brightens and the grille glows
//
// Pack: eidoverse/assets/models/voice_machines/ (README.md, SOURCES.md).
// Geometry and materials come from the pack's speakspell.glb, modelled and baked in Blender by
// blender/build_speakspell.py (one-mass shell with the handle hole, perimeter parting line, card pocket, battery
// door, screw pockets; the black band as its own moulding with real grille slots into a speaker cavity and a
// display window; a printed membrane card with 40 embossed domes; wire plate stand). AmbientCG Plastic013B /
// Plastic012B / Rubber004 scans box-projected at real scale, AO grime, edge wear and fading baked to 2048 maps.
// Real size (V&A): 254 × 177 × 38 mm. Metres, +Y up, resting on y = 0, facing +Z; leans back 0.12 rad on its
// stand (stand: false → upright, no stand).
// parts: toy, body, card, stand, display {mesh, setText, text, power, screen}, speaker {mesh, set(v)}.
// Draw calls: 6 (body, card, screws, stand, VFD glass, grille glow).

export async function build(THREE, opts = {}) {
    const { text = 'SPELL', stand = true } = opts;
    const K = await eraKit(THREE);
    const { uniform } = THREE;
    const disposables = [];
    const track = (x) => { disposables.push(x); return x; };
    const T = 0.038, LEAN = stand ? 0.12 : 0, WR = 0.0016;
    const VFD = { w: 0.0676, h: 0.0138 };

    const root = await K.loadGLB(new URL('../../assets/models/voice_machines/speakspell.glb', import.meta.url));
    const group = new THREE.Group(); group.name = 'speakspell';
    const toy = new THREE.Group(); toy.name = 'speakspell_toy';
    group.add(toy);
    const byName = {};
    root.traverse((o) => { if (o.isMesh) byName[o.name] = o; });
    const need = (n) => { if (!byName[n]) throw new Error('[speakspell] GLB is missing ' + n); return byName[n]; };

    // baked meshes
    const body = need('ss_body'), card = need('ss_card'), standMesh = need('part_stand'), screws = need('metal_screws');
    for (const m of [body, card, standMesh]) { m.material = track(K.bakedMaterial(m.material)); m.castShadow = m.receiveShadow = true; }
    screws.material = track(K.metalMaterial('#a9a9ad', 0.34));

    // the VFD behind its glass (makeScreen canvas → emissive under a clear coat, recessed by parallax)
    const power = uniform(1), gain = uniform(1);
    const vfdState = { text: '', align: 'left', dirty: true };
    const vfd = K.eraScreen({ width: VFD.w, height: VFD.h, px: 1024, draw: (ctx, t, w, h) => drawVFD(ctx, w, h, vfdState.text, vfdState.align) });
    track(vfd.tex);
    const glass = need('screen_vfd');
    K.flipUV(glass.geometry);
    glass.material = track(K.screenMaterial({
        tex: vfd.tex, power: power.mul(gain), gain: 1.2, flipV: true, parallax: 0.0032, faceSize: [VFD.w, VFD.h],
        glass: '#07090a', rough: 0.3, coatRough: 0.025,
    }));
    // the speaker cavity wall: lit by the voice through the grille slots
    const glowMesh = need('glow_grille');
    const G = K.glowMaterial('#7ff6ff');
    glowMesh.material = track(G.mat);

    for (const m of [body, card, screws, glass, glowMesh]) toy.add(m);
    if (stand) group.add(standMesh);

    // lean back onto the stand exactly as the Blender build placed it (same LEAN, same lift)
    toy.rotation.x = -LEAN;
    toy.position.y = stand ? 2 * WR + (T / 2) * Math.sin(LEAN) + 0.0005 : 0;
    if (!stand) { toy.updateMatrixWorld(true); toy.position.y = -new THREE.Box3().setFromObject(toy).min.y; }

    // ── API
    const setText = (s, o = {}) => {
        const t = String(s ?? '').toUpperCase().slice(0, 8), a = o.align || 'left';
        if (t !== vfdState.text || a !== vfdState.align) { vfdState.text = t; vfdState.align = a; vfdState.dirty = true; }
    };
    setText(text);
    vfd.redraw(0); vfdState.dirty = false;
    const speaker = { mesh: glowMesh, set(v) { G.level.value = Math.max(0, v); } };
    const parts = {
        toy, body, card, stand: stand ? standMesh : null,
        display: { mesh: glass, setText, get text() { return vfdState.text; }, power, screen: vfd.scr },
        speaker,
    };
    function update(t, state = {}) {
        const p = Math.max(0, state.power ?? 1), v = Math.max(0, Math.min(1, state.voice ?? 0));
        power.value = p;
        gain.value = 1 + 0.55 * v;                 // the tube flares a little on each syllable
        speaker.set(p > 0.01 ? v : 0);
        if (state.text !== undefined) setText(state.text, state);
        if (vfdState.dirty) { vfd.redraw(t); vfdState.dirty = false; }
    }
    function dispose() {
        group.traverse((o) => { if (o.isMesh) o.geometry.dispose(); });
        for (const d of disposables) d.dispose?.();
        vfd.scr.mesh.geometry.dispose(); vfd.scr.material.dispose();
    }
    return { group, parts, update, dispose };

    // ── 14-segment VFD (the toy's 8-character vacuum-fluorescent tube), drawn on canvas ──
    function drawVFD(ctx, w, h, str, align) {
        ctx.save();
        ctx.fillStyle = '#000'; ctx.fillRect(0, 0, w, h);
        const N = 8, cellW = w / N, cellH = h * 0.8, top = h * 0.1;
        const chars = String(str || '').split('');
        const off = align === 'right' ? N - chars.length : align === 'center' ? Math.floor((N - chars.length) / 2) : 0;
        const SEG = {
            a: [[0.14, 0.07], [0.86, 0.07]], d: [[0.14, 0.93], [0.86, 0.93]],
            g1: [[0.14, 0.5], [0.5, 0.5]], g2: [[0.5, 0.5], [0.86, 0.5]],
            f: [[0.12, 0.07], [0.12, 0.5]], e: [[0.12, 0.5], [0.12, 0.93]],
            b: [[0.88, 0.07], [0.88, 0.5]], c: [[0.88, 0.5], [0.88, 0.93]],
            i: [[0.5, 0.07], [0.5, 0.5]], l: [[0.5, 0.5], [0.5, 0.93]],
            h: [[0.12, 0.07], [0.5, 0.5]], j: [[0.88, 0.07], [0.5, 0.5]],
            k: [[0.12, 0.93], [0.5, 0.5]], m: [[0.88, 0.93], [0.5, 0.5]],
        };
        const GLYPH = {
            A: 'abcefg1g2', B: 'abcdilg2', C: 'adef', D: 'abcdil', E: 'adefg1g2', F: 'aefg1', G: 'acdefg2', H: 'bcefg1g2',
            I: 'adil', J: 'bcde', K: 'efg1jm', L: 'def', M: 'bcefhj', N: 'bcefhm', O: 'abcdef', P: 'abefg1g2',
            Q: 'abcdefm', R: 'abefg1g2m', S: 'acdfg1g2', T: 'ail', U: 'bcdef', V: 'efjk', W: 'bcefkm', X: 'hjkm',
            Y: 'hjl', Z: 'adjk', 0: 'abcdefjk', 1: 'bcj', 2: 'abdeg1g2', 3: 'abcdg2', 4: 'bcfg1g2', 5: 'acdfg1g2',
            6: 'acdefg1g2', 7: 'abc', 8: 'abcdefg1g2', 9: 'abcdfg1g2', '-': 'g1g2', "'": 'j', '?': 'abg2l', '!': 'il',
            '*': 'g1g2hijklm', '+': 'g1g2il', '/': 'jk', '_': 'd', ' ': '', '=': 'dg1g2', '<': 'jm', '>': 'hk',
        };
        const segList = (code) => (code.match(/g1|g2|[a-fh-m]/g) || []);
        const drawSeg = (x0, y0, x1, y1, hw, gap) => {
            const dx = x1 - x0, dy = y1 - y0, L = Math.hypot(dx, dy), ux = dx / L, uy = dy / L, nx = -uy, ny = ux;
            const ax = x0 + ux * gap, ay = y0 + uy * gap, bx = x1 - ux * gap, by = y1 - uy * gap;
            ctx.beginPath();
            ctx.moveTo(ax, ay);
            ctx.lineTo(ax + ux * hw + nx * hw, ay + uy * hw + ny * hw);
            ctx.lineTo(bx - ux * hw + nx * hw, by - uy * hw + ny * hw);
            ctx.lineTo(bx, by);
            ctx.lineTo(bx - ux * hw - nx * hw, by - uy * hw - ny * hw);
            ctx.lineTo(ax + ux * hw - nx * hw, ay + uy * hw - ny * hw);
            ctx.closePath(); ctx.fill();
        };
        const cellSegs = (ci, list, pass) => {
            const cx = ci * cellW + cellW * 0.1, cwid = cellW * 0.8, hw = cellH * 0.052;
            for (const s of list) {
                const [[a0, b0], [a1, b1]] = SEG[s];
                const diag = s === 'h' || s === 'j' || s === 'k' || s === 'm';
                drawSeg(cx + a0 * cwid, top + b0 * cellH, cx + a1 * cwid, top + b1 * cellH, diag ? hw * 0.75 : hw, diag ? cellH * 0.07 : cellH * 0.028);
            }
            if (pass === 'ghost' || list.dp) { ctx.beginPath(); ctx.arc(cx + cwid * 1.03, top + cellH * 0.95, hw * 0.9, 0, Math.PI * 2); ctx.fill(); }
        };
        ctx.fillStyle = 'rgba(62,242,228,0.08)';
        for (let ci = 0; ci < N; ci++) cellSegs(ci, Object.keys(SEG), 'ghost');
        const litCells = [];
        for (let i = 0; i < chars.length; i++) {
            const ch = chars[i], list = segList(GLYPH[ch] ?? '');
            if (ch === '.' || ch === ',' || ch === '!') list.dp = true;
            litCells.push([off + i, list]);
        }
        ctx.shadowColor = 'rgba(16,214,204,0.9)'; ctx.shadowBlur = cellH * 0.2; ctx.fillStyle = '#22d8cc';
        for (const [ci, list] of litCells) cellSegs(ci, list, 'lit');
        ctx.shadowBlur = cellH * 0.06; ctx.fillStyle = '#4cf2e2';
        for (const [ci, list] of litCells) cellSegs(ci, list, 'lit');
        ctx.shadowBlur = 0; ctx.globalAlpha = 0.28; ctx.fillStyle = '#b4fff6';
        for (const [ci, list] of litCells) cellSegs(ci, list, 'lit');
        ctx.globalAlpha = 1;
        ctx.strokeStyle = 'rgba(0,0,0,0.30)'; ctx.lineWidth = 1;          // control grid
        const pitch = Math.max(3, Math.round(cellH * 0.045));
        ctx.beginPath();
        for (let x = -h; x < w + h; x += pitch) { ctx.moveTo(x, 0); ctx.lineTo(x + h, h); ctx.moveTo(x + h, 0); ctx.lineTo(x, h); }
        ctx.stroke();
        ctx.strokeStyle = 'rgba(0,0,0,0.55)'; ctx.lineWidth = 1.5;          // filaments
        for (const fy of [0.22, 0.5, 0.78]) { ctx.beginPath(); ctx.moveTo(0, h * fy); ctx.lineTo(w, h * fy + 1); ctx.stroke(); }
        ctx.restore();
    }
}
