...
The runner writes the brief and capability files, mounts the repo at
`/workspace` (engine read-only), launches the agent CLI inside the
...
- **`auto`** (this branch) — the containerized edition: a Docker render
  image and a runner that wraps the toolkit as a **subagent for
  autonomous agentic loops** (a parent orchestrator hands in a brief
  file and gets back a finished mp4, engine mounted read-only).
- **`main`** — the harness edition: Deno + ffmpeg on your machine, no
  containers anywhere.
